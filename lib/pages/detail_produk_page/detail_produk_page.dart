...
class _DetailProdukPageState extends State<DetailProdukPage> {
  ...
  void getData() async {
    await getCubit.fetch(Repositories.getSingleProduk, ProdukRes(kode: widget.kode));
  }

  void getKeranjangData() async {
    if (widget.kodeKeranjang == null) return;
    final res = await getKeranjangCubit.fetch(Repositories.getSingleKeranjang, KeranjangRes(kode: widget.kodeKeranjang));
  ...
  void updateKeranjangData() async {
    if (widget.kodeKeranjang == null) return;
    final res = await getKeranjangCubit.fetch(Repositories.updateKeranjang, KeranjangRes(kode: widget.kodeKeranjang, catatan: catatanCon.text, total: total));
    if (res is BaseLogicSuccess<BaseResponse<KeranjangRes>>) {
      context.pop();
    }
  }
  ...
  void addKeranjang() async {
    ...
    if (res is BaseLogicSuccess<BaseResponse<KeranjangRes>>) {
      context.read<BaseLogic<List<KeranjangRes>>>().fetch(Repositories.getAllKeranjangByUser, locator<SettingCubit>().state.kd!);
      InfoCard(
        InfoType.success,
      ...
      child: BaseBlocBuilder(
        ...
        onSuccess: (cubit, data) => Scaffold(
              ...
              children: [
                ...
                Positioned(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(image: DecorationImage(fit: BoxFit.cover, image: NetworkImage(locator<Htreq>().base.baseUrl + (data.data?.foto ?? "")))),
                    )),
                //icon widget
                Positioned(
                    ...
                    child: Row(
                      ...
                      children: [
                        GestureDetector(onTap: () => context.pop(), child: const AppIcon(icon: Icons.arrow_back_ios)),
                                ...
                                Positioned(
                                  ...
                                  child: BaseBlocBuilder(
                                    ...
                                    onSuccess: (cubit, data) => BigText(
                                      text: data.data?.length.toCurrency() ?? "0",
                                      size: 12,
                                      color: Colors.white,
                                    ),
                                  ),
                ...
                Positioned(
                    ...
                    right: 0,
                    bottom: 0,
                    top: (Dimensions.popularFoodImgSize(context) - 20) - MediaQuery.of(context).viewInsets.bottom,
                    child: Container(
                      padding: EdgeInsets.only(left: Dimensions.width20(context), right: Dimensions.width20(context), top: Dimensions.height20(context)),
                      decoration: BoxDecoration(borderRadius: BorderRadius.only(topRight: Radius.circular(Dimensions.radius20(context)), topLeft: Radius.circular(Dimensions.radius20(context))), color: Colors.white),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          AppColumn(text: data.data?.nama ?? "-", est: data.data?.estimasi ?? 0),
                          SizedBox(
                            height: Dimensions.height10(context),
                          ),
                          MainText(
                            "Rp ${data.data?.harga.toCurrency()}",
                            style: TextStyle(color: AppColors.iconColor2, fontSize: context.fontSize(25)),
                          ),
                          const Divider(
                            thickness: 5,
                          ),
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Align(
                                  ...
                                  child: IconAndTextWidget(icon: Icons.access_time_rounded, text: "${data.data?.estimasi.toCurrency()} min", iconColor: AppColors.iconColor2),
                                ),
                              )
                            ],
                          ),
                          ...
                          Expanded(child: SingleChildScrollView(child: ExpandableTextWidget(text: data.data?.deskripsi ?? "-"))),
                          MainTextField(
                            label: "Catatan",
            ...
            bottomNavigationBar: Container(
              ...
              decoration: BoxDecoration(color: AppColors.buttonBackgroundCOlor, borderRadius: BorderRadius.only(topLeft: Radius.circular(Dimensions.radius20(context) * 2), topRight: Radius.circular(Dimensions.radius20(context) * 2))),
              child: Row(
                children: [
                  Expanded(
                    child: Container(
                      ...
                      decoration: BoxDecoration(borderRadius: BorderRadius.circular(Dimensions.radius20(context)), color: Colors.white),
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.spaceAround,
                        children: [
                          InkWell(onTap: () => decrease(), child: const Icon(Icons.remove, color: AppColors.signColor)),
                          SizedBox(
                          ...
                          InkWell(onTap: () => increase(), child: const Icon(Icons.add, color: AppColors.signColor))
                        ],
                      ),
                    ),
                  ),
                    ...
                    child: Builder(builder: (context) {
                      ...
                      return InkWell(
                        ...
                        child: Container(
                          ...
                          padding: EdgeInsets.only(top: Dimensions.height20(context), bottom: Dimensions.height20(context), left: Dimensions.width15(context), right: Dimensions.width15(context)),
                          decoration: BoxDecoration(borderRadius: BorderRadius.circular(Dimensions.radius20(context)), color: AppColors.mainColor),
                          child: BigText(
                            text: dataBuilder(() {
                              if (widget.kodeKeranjang != null) {
